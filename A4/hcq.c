 ...
 */
Student *take_student(Ta *ta, Student **stu_list_ptr, Student *to_serve) {
...
char* print_currently_serving(Ta *ta_list) {
    char *fq = malloc(OUT_BUF_SIZE);
    fq[0] = '\0';
    memset(fq, '\0', OUT_BUF_SIZE);
    char buf[OUT_BUF_SIZE];
    int space = OUT_BUF_SIZE;
    if (ta_list == NULL) {
	strcpy(fq, "No TAs are currently working.\r\n");
    ...
    while (ta_list != NULL) {
       if (ta_list->current_student != NULL) {
	    ...
	    sprintf(buf, "TA: %s is serving %s.\r\n", ta_list->name, ta_list->current_student->name);

       } else {
           //printf("TA: %s has no student\r\n", ta_list->name);
	    sprintf(buf, "TA: %s has no student\r\n", ta_list->name);
       }
	strncat(fq, buf, space);
	space = space - strlen(buf);
	if(space < 0){
            space = 0;
        }
...
char* print_full_queue(Student *stu_list) {
    char *fq = malloc(OUT_BUF_SIZE);
    memset(fq, '\0', OUT_BUF_SIZE);
    strcpy(fq, "Full Queue\r\n");
    char buf[OUT_BUF_SIZE];
    int space = OUT_BUF_SIZE - sizeof("Full Queue\r\n");
    //printf("Full Queue\n");
    while (stu_list != NULL) {
	sprintf(buf, "Student %s:%s\r\n",stu_list->name, stu_list->course->code);
	strncat(fq, buf, space);
	space = space - strlen(buf);
	if(space < 0){
	    space = 0;
